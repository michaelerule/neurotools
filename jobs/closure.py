...
def verify_function_closure(f):
    ...
    for k,v in bound.items():
        if not is_probably_safe(v):
            raise ValueError('Function %s is not safe for memoization/caching, it binds a mutable %s %s!'%(f.func_name,k,v))
    ...
    if f.func_closure:
        closed = dict(zip(f.__code__.co_freevars,f.func_closure))
        # Make sure closed-over and bound variables are... reasonable
        for k,v in closed.items():
            if not is_probably_safe(v):
                raise ValueError('Function %s is not safe for memoization/caching, it closes over the mutable variable %s %s!'%(f.func_name,k,v))
        
    # Make sure all default arguments are immutable
    if f.func_defaults:
        for v in f.func_defaults:
            if not is_probably_safe(v):
                raise ValueError('Function %s is not safe for memoization/caching, it uses a mutable default %s!'%(f.func_name,v))
    
    print('Function %s seems ok'%f.func_name)
    return True
