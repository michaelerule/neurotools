...
'''
Miscellaneous routines from lessons that have not found a better home
'''
...
def sample_ou_process(x0,sigma,tau,dt,N,ntrial=1):
    ...
    for i in range(N):
        x += -(1./tau)*x*dt + sigma * randn(ntrial) * sqrt(dt)
        simulated[i] = x
    return simulated
...
def precision1D(N,a,b):
    ...
    precision = diag(ones(N)*a)*2 - diag(ones(N-1)*b,-1) - diag(ones(N-1)*b,1)
    return precision
...
def showim(x,**vargs):
    ...
    if x.dtype in (np.complex64,np.complex128,np.complex256):
        ...
        subplot(131)
        imshow(x.real,vmin=vmin,vmax=vmax,**vargs)
        subplot(132)
        imshow(x.imag,vmin=vmin,vmax=vmax,**vargs)
        subplot(133)
        imshow(abs(x),vmin=vmin,vmax=vmax,**vargs);
    else:
        # real-valued matrix
        imshow(x,**vargs)
...
def infer_states_Gaussian_ADF(Y,variance,true_states=None,do_plot=True):
    fx0 = Gaussian(0,0)
    fA  = DiffusionGaussian(variance)
    fB  = PoissonObservationModel(1,0)
    L = len(Y)
    ...
    if do_plot:
        figure()
        plot(m,color='k')
        fill_between(range(L),m-s*1.95,m+s*1.95,color=(0.1,)*4,lw=0)
        if not true_states is None:
            plot(true_states,color='r');
        xlim(0,L)
    return m,v
...
class PGOM:
    ...
    '''
    class Approximate(PoissonObservationApproximator):
